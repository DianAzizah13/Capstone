...
const getIngredient = async (req, res) => {
  try {
    const { keyword, price, latitude, longitude } = req.query;

    const priceConditions = {
      murah: { [Op.lt]: 10000 },
      sedang: { [Op.between]: [10000, 20000] },
      mahal: { [Op.gt]: 20000 },
    };
    ...
    if (keyword) {
      whereCondition.ingredient_nama = {
        [Op.like]: `%${keyword}%`,
      };
    }

    if (price && priceConditions[price]) {
      whereCondition.ingredient_harga = priceConditions[price];
    }

    if (latitude && longitude) {
      whereCondition['$Toko.toko_latitude$'] = {
        [Op.between]: [Number(latitude) - 0.05, Number(latitude) + 0.05],
      };
      whereCondition['$Toko.toko_longitude$'] = {
        [Op.between]: [Number(longitude) - 0.05, Number(longitude) + 0.05],
      };
    }

    // Join tabel 'ingredients' dengan tabel 'tokos' berdasarkan toko_id
    ...
    console.error('Terjadi kesalahan saat mencari resep:', error);
    return res.status(500).json({ error: 'Terjadi kesalahan saat mencari resep' });
  }
};
...
const store = async (req, res) => {
  try {
    ...
    const { ingredient_nama, ingredient_jenis, ingredient_deskripsi } =
      req.body;
  ...
  try {
    ...
    const response = {
      ...
      message: data
        ...
        : `Data Ingredient dengan ID ${id} tidak ditemukan`,
      data: result,
    };
    ...
    if (req.file) {
      ...
      if (oldImageUrl) {
        const imagePath = path.join(
          ...
          "..",
          "public/ingredients/",
          getFileNameFromUrl(oldImageUrl)
        );
    ...
    await ingredient.update(
      {
        ...
        ingredient_deskripsi:
          ingredient_deskripsi || ingredient.ingredient_deskripsi,
      },
      {
        fields: [
          "ingredient_nama",
